 ...
 *
 * HAL Member Profiles — canonical managed Profile template source (Elementor bridge).
 *
 * This file is HAL's own source of record for the managed Ultimate Member Profile
 * template. It lives inside the plugin so it ships in every release; the copy that
 * Ultimate Member actually loads is provisioned into the active Child Theme from this
 * source by HAL's managed-template system. Selection stays an explicit admin decision:
 * after provisioning, choose "HAL Member Profiles" as the template for the relevant
 * Ultimate Member Profile Form(s) from Ultimate Member > Forms. This file never selects
 * itself, never replaces Ultimate Member's own profile.php, and never removes or edits
 * any other template.
 *
 * Thin bridge only: no CSS, no "Hero" markup, no queries, no privacy logic, and no
 * Amelia logic live here — all of that belongs to the plugin's own PHP classes. In
 * Public Profile this file never calls um_profile_header_cover_area, um_profile_header,
 * um_profile_navbar, um_profile_menu, or um_profile_content_* directly; it calls only
 * \HAL\MemberProfiles\Integrations\ProfileLayoutAdapter::render_or_fallback() with a
 * callback that reproduces Ultimate Member's own complete native pipeline. When the
 * layout mode is observe, the executive compatibility gate does not pass for this
 * composition, or the adapter class is absent (HAL inactive), that native pipeline runs
 * directly instead.
 *
